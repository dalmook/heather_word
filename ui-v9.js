...
const TAB_META = Object.freeze({
  ...
  learn: { label: "단어", icon: "book" },
  games: { label: "놀이", icon: "game" },
  collection: { label: "친구", icon: "collection" },
  my: { label: "나", icon: "user" }
});
...
function formatNumber(value) { return new Intl.NumberFormat("ko-KR").format(safeInt(value)); }
function greeting() {
  const hour = new Date().getHours();
  if (hour < 11) return "좋은 아침!";
  if (hour < 17) return "오늘도 반가워!";
  return "만나서 반가워!";
}
...
function legacyBridge() { return window.HeatherWordLegacyBridge || null; }
function installStorageSignal() {
  ...
  Storage.prototype.setItem = function hw9SetItem(key, value) {
    ...
    if (this === window.localStorage && key === LOCAL_KEY) window.dispatchEvent(new CustomEvent("heather:state-change"));
    return result;
  };
}
function waitForBaseApp(timeout = 12000) {
  ...
  return new Promise((resolve) => {
    const tick = () => {
      if (window.HeatherWordLegacyBridge && document.querySelector("#homeScreen") && document.querySelector("#cardScreen") && document.querySelector("#gameScreen")) { resolve(true); return; }
...
function shellTemplate() {
  return `
    ...
    <header class="hw9-header">
      <button class="hw9-profile-button" type="button" data-hw9-action="profile">
    ...
    <dialog class="hw9-parent-dialog" id="hw9ParentDialog" aria-labelledby="hw9ParentTitle">
      <form method="dialog" class="hw9-parent-sheet" data-hw9-parent-form>
        ...
        <p class="hw9-eyebrow">보호자 도구</p><h2 id="hw9ParentTitle">보호자 PIN 확인</h2>
        <p data-hw9-parent-description>단어장 편집과 데이터 관리 화면을 열어요.</p>
        <label class="hw9-field"><span>PIN</span><input data-hw9-parent-pin type="password" inputmode="numeric" pattern="[0-9]*" minlength="4" maxlength="8" autocomplete="off" placeholder="숫자 4~8자리" /></label>
...
function mountShell() {
  ...
  root.id = "hw9App"; root.className = "hw9-app"; root.dataset.version = UI_V9_VERSION;
  root.innerHTML = shellTemplate(); document.body.appendChild(root); return root;
}
function partnerMarkup(snapshot, size = "large") {
  const partner = getSeason2Character(snapshot.partnerId);
  if (partner) return `<div class="hw9-partner-art is-${size}" role="img" aria-label="함께하는 친구 ${escapeHtml(partner.name)}">${renderMonsterSvg(partner)}</div>`;
  const latestLegacy = document.querySelector("#avatarPreview")?.innerHTML?.trim();
  if (latestLegacy && latestLegacy.includes("svg")) return `<div class="hw9-partner-art is-${size}" role="img" aria-label="현재 캐릭터">${latestLegacy}</div>`;
  return `<div class="hw9-partner-art is-${size}" role="img" aria-label="탐험 안내 친구. 아직 내 파트너를 고르기 전이에요.">${renderMonsterSvg(getSeason2Character("s2_cookie_crumb_bear_1"))}</div>`;
}
...
function categoryRows(snapshot) {
  ...
  if (!visible.length) return `<div class="hw9-empty-state">${icon("book")}<h3>아직 단어가 없어요</h3><p>보호자 도구에서 첫 단어를 추가하면 바로 학습할 수 있어요.</p><button class="hw9-button hw9-button-secondary" type="button" data-hw9-action="parent">단어 추가하기</button></div>`;
  return `<div class="hw9-category-list">${visible.map((category) => `<button type="button" class="hw9-category-row" data-hw9-category="${escapeHtml(category.id)}"><span class="hw9-category-index" aria-hidden="true">${category.emoji ? escapeHtml(category.emoji) : icon("book")}</span><span><strong>${escapeHtml(category.name)}</strong><small>${category.count}개 단어</small></span>${icon("play")}</button>`).join("")}</div>${categories.length > 6 ? `<button type="button" class="hw9-text-button" data-hw9-action="toggle-categories">${app.showAllCategories ? "간단히 보기" : `카테고리 전체 ${categories.length}개 보기`} ${icon("chevron")}</button>` : ""}`;
}
function renderWordLibrary(snapshot) {
  return `<section class="hw9-view hw9-library-view" aria-labelledby="hw9LibraryTitle">
    <div class="hw9-view-heading with-back"><button type="button" class="hw9-icon-button" data-hw9-action="learn-overview" aria-label="학습 화면으로 돌아가기">${icon("back")}</button><div><span class="hw9-kicker">내 단어장</span><h1 id="hw9LibraryTitle">${snapshot.wordCount}개 단어</h1></div></div>
    <div class="hw9-library-tools"><label class="hw9-search-field">${icon("search")}<input type="search" data-hw9-word-search value="${escapeHtml(app.wordQuery)}" aria-label="단어 또는 뜻 검색" placeholder="단어 또는 뜻 검색" autocomplete="off" /></label><select data-hw9-word-category aria-label="카테고리 필터">${snapshot.categories.filter(c=>c.id==="all"||c.count>0).map(c=>`<option value="${escapeHtml(c.id)}" ${c.id===app.wordCategory?'selected':''}>${escapeHtml(c.name)} · ${c.count}</option>`).join('')}</select></div>
    <div data-hw9-results>${renderWordResults(snapshot)}</div><button class="hw9-button hw9-button-secondary hw9-button-wide" type="button" data-hw9-action="parent">${icon("lock")} 보호자 단어 관리</button></section>`;
}
function renderWordResults(snapshot) {
  const result=wordResults(snapshot.words, app.wordQuery, app.wordCategory, app.wordLimit);
  return `<p class="hw9-result-count" role="status">검색 결과 ${result.total}개 · ${result.items.length}개 표시</p><div class="hw9-word-list">${result.items.length?result.items.map(word=>`<article class="hw9-word-row"><div><strong>${escapeText(word.word)}</strong><span>${escapeText(word.meaning||'뜻이 아직 없어요')}</span></div><button type="button" class="hw9-icon-button subtle" data-hw9-speak="${escapeText(word.word)}" aria-label="${escapeText(word.word)} 발음 듣기">${icon("volume")}</button></article>`).join(''):`<div class="hw9-empty-state">${icon("search")}<h3>찾는 단어가 없어요</h3><p>검색어나 카테고리를 바꿔 보세요.</p><button type="button" class="hw9-text-button" data-hw9-action="clear-search">검색 초기화</button></div>`}</div>${result.hasMore?`<button type="button" class="hw9-text-button hw9-button-wide" data-hw9-action="more-words">다음 단어 더 보기 (${result.items.length}/${result.total})</button>`:''}`;
}
...
function renderGames() {
  const s = app.snapshot;
  const selected = availableCategory(s);
  return `<section class="hw9-view hw9-games-view" aria-labelledby="hw9GamesTitle">
      <div class="hw9-view-heading"><div><span class="hw9-kicker">단어 놀이터</span><h1 id="hw9GamesTitle">어떤 놀이를 할까?</h1><p>하나를 골라 봐. 틀려도 괜찮아!</p></div></div>
      <button type="button" class="hw9-adventure-banner" data-hw9-action="adventure" ${s.wordCount ? "" : "disabled"}><span>${icon("sparkles")}</span><div><strong>오늘의 모험</strong><small>4단계 · 총 21문제 · 진행 상황 저장</small></div><em>${s.adventure.completed ? "완료" : `${s.adventure.stageIndex}/4`}</em>${icon("chevron")}</button>
      <label class="hw9-field hw9-game-category"><span>어떤 단어로 놀까?</span><select data-hw9-game-category aria-label="게임 카테고리">${s.categories.filter(c=>c.id==="all"||c.count>0).map(c=>`<option value="${escapeHtml(c.id)}" ${c.id===selected?'selected':''}>${escapeHtml(c.name)} · ${c.count}개</option>`).join('')}</select></label>
      <div class="hw9-game-grid">${GAME_META.map((game) => `<button type="button" class="hw9-game-card mode-${game.mode}" data-hw9-game="${game.mode}" ${s.wordCount ? "" : "disabled"}><span class="hw9-game-icon" aria-hidden="true">${gameArtwork(game.mode)}</span><span class="hw9-game-level">${game.level}</span><strong>${game.title}</strong><p>${game.description}</p><small>10문제 · ${game.level} ${icon("arrow")}</small></button>`).join("")}</div>
      ${!s.wordCount ? `<div class="hw9-inline-notice">${icon("offline")}<span>학습할 단어가 없어서 게임을 시작할 수 없어요.</span><button type="button" data-hw9-action="parent">단어 추가</button></div>` : ""}</section>`;
}
function ownedPreview(snapshot) {
  ...
  if (!owned.length) return `<div class="hw9-collection-empty">${icon("monster")}<span>모험을 완료하면 새로운 친구가 이곳에 나타나요.</span></div>`;
...
function renderCollection() {
  ...
  return `<section class="hw9-view hw9-collection-view" aria-labelledby="hw9CollectionTitle">
      <div class="hw9-view-heading"><div><span class="hw9-kicker">친구들의 마을</span><h1 id="hw9CollectionTitle">우리 같이 자라자!</h1><p>친구를 만나고, 돌보고, 꾸며 봐.</p></div></div>
      <article class="hw9-partner-feature"><div class="hw9-partner-scene">${partnerMarkup(s)}<i></i></div><div class="hw9-partner-copy"><span class="hw9-kicker">함께하는 친구</span><h2>${escapeHtml(partner?.name || "첫 친구를 골라 봐!")}</h2><p>${escapeHtml(partner?.personality || `${world?.name || "스펠링 월드"}에서 첫 친구를 기다리고 있어요`)}</p><button type="button" class="hw9-button hw9-button-secondary" data-hw9-action="season2-collection">파트너와 도감 보기</button></div></article>
      <section class="hw9-section-block"><div class="hw9-section-title"><div><span class="hw9-kicker">최근 친구</span><h2>시즌 2 도감</h2></div><span>${s.ownedSeason2}/60</span></div>${progressBar((s.ownedSeason2 / 60) * 100, "시즌 2 수집률")}${ownedPreview(s)}</section>
      <div class="hw9-world-menu">
        ...
        <button type="button" data-hw9-legacy="collection"><span>${icon("collection")}</span><div><strong>몬스터 친구들</strong><small>차곡차곡 모은 1,000종 친구</small></div><em>${escapeHtml(legacyCount)}/1000</em>${icon("chevron")}</button>
        <button type="button" data-hw9-legacy="pet"><span>${icon("pet")}</span><div><strong>펫 돌보기</strong><small>밥도 주고 함께 놀아 줘</small></div>${icon("chevron")}</button>
        <button type="button" data-hw9-legacy="dress"><span>${icon("shirt")}</span><div><strong>내 모습 꾸미기</strong><small>내 모습과 배경을 꾸며요</small></div>${icon("chevron")}</button>
      </div></section>`;
}
function settingRow(iconName, title, description, action, trailing = "") {
  return `<button type="button" class="hw9-setting-row" ${action}><span class="hw9-setting-icon">${icon(iconName)}</span><span><strong>${title}</strong><small>${description}</small></span>${trailing ? `<em>${trailing}</em>` : ""}${icon("chevron")}</button>`;
}
function renderMy() {
  ...
  return `<section class="hw9-view hw9-my-view" aria-labelledby="hw9MyTitle">
      ...
      <section class="hw9-settings-group"><h2>앱 설정</h2>
        ${settingRow(s.sound ? "volume" : "mute", "소리", s.sound ? "효과음과 발음이 켜져 있어요" : "소리가 꺼져 있어요", 'data-hw9-action="toggle-sound"', s.sound ? "켜짐" : "꺼짐")}
        ${settingRow("settings", "나에게 맞게", "쉬움·도전, 소리와 움직임 설정", 'data-hw9-action="season2-settings"')}
        ${settingRow("sync", "데이터 동기화", connection.label, 'data-hw9-action="sync"')}
      </section>
      <section class="hw9-settings-group hw9-parent-entry"><h2>보호자</h2>
...
function renderActiveView({ force = false, preserveScroll = false } = {}) {
  ...
  app.main.innerHTML = renderers[app.activeTab](); app.main.scrollTop = preserveScroll ? previousScroll : 0; app.root.dataset.tab = app.activeTab;
...
function refreshSnapshot({ force = false } = {}) {
  ...
  if(changed||force) {
    ...
    if(app.activeTab==='learn'&&app.learnMode==='library'&&app.main.querySelector('[data-hw9-results]')) {
      refreshWordResults(); const title=app.main.querySelector('#hw9LibraryTitle'); if(title) title.textContent=`${next.wordCount}개 단어`;
    } else if(!app.root.hidden && !document.querySelector('dialog[open]') && !document.body.classList.contains('hw9-season2-open')) renderActiveView({preserveScroll:true});
  }
  applySettings();
}
function scheduleRefresh() {
  if(app.refreshPending) return;
  app.refreshPending=true; requestAnimationFrame(()=>{app.refreshPending=false;refreshSnapshot();});
}
function applySettings() {
  if (!app.snapshot) return;
  const reduced = app.snapshot.settings.reducedMotion || matchMedia("(prefers-reduced-motion: reduce)").matches;
  document.documentElement.classList.toggle("hw9-reduced-motion", reduced); document.body.dataset.hw9AgeBand = app.snapshot.settings.ageBand;
}
function announce(message) {
  const region=app.root?.querySelector('[data-hw9-live]'); if(region) region.textContent=message;
  const toast=app.root?.querySelector('[data-hw9-toast]'); if(!toast) return;
  toast.textContent=message; toast.hidden=false; clearTimeout(app.toastTimer); app.toastTimer=setTimeout(()=>{toast.hidden=true;},3600);
}
function setTab(tab, { history = true, focus = true } = {}) {
  if(activeLegacyScreen()) {legacyBridge()?.navigate('home');syncLegacyVisibility();}
  if(document.querySelector('#season2Overlay')?.hidden===false) {const previous=app.handlingHistory;app.handlingHistory=true;window.HeatherWordSeason2?.close?.();app.handlingHistory=previous;}
  const next = normalizeTab(tab); app.activeTab = next; app.learnMode = next === "learn" ? app.learnMode : "overview";
  if (history) {const hash = `#/${next}`;if (location.hash !== hash) historyPush({ hw9: true, tab: next }, hash);}
  renderActiveView({ force: focus });
}
function historyPush(state, hash) { try { history.pushState(state, "", hash); } catch { location.hash = hash; } }
function handleHistory() {
  ...
  if(season) {
    ...
    if(window.HeatherWordSeason2) {window.HeatherWordSeason2.open(season[1]);app.handlingHistory=false;}
    else {app.handlingHistory=false;app.lastRoute='';setTimeout(handleHistory,100);}
    return;
  }
...
function openAdventure() {
  ...
  if (button) {button.click();announce("오늘의 모험을 열었어요");return;}
  openLegacy("card");
}
function openSeason2View(view) {
  ...
  openAdventure(); setTimeout(() => document.querySelector(`#season2Overlay [data-s2-view="${view}"]`)?.click(), 60); return false;
}
function openReport() {
  const button = document.querySelector('#season2HomePanel [data-s2-action="open-report"]');
  if (button) button.click(); else openSeason2View("weekly");
}
function openLegacy(screen, options = {}) {
  if (!(screen in LEGACY_SCREENS)) return;
  if (screen === "manage" && window.HEATHER_PARENT_GATE_GRANTED !== true) {openParentGate();return;}
  ...
  if (screen === "game" && options.mode && bridge?.startGame) bridge.startGame(options.mode, options.categoryId || app.snapshot.selectedCategoryId);
  else if (screen === "card" && bridge?.startCard) bridge.startCard(options.categoryId || app.snapshot.selectedCategoryId);
  else if (bridge?.navigate) bridge.navigate(screen);
  else {
    ...
    if (options.categoryId) {
      ...
      if (select) {select.value = options.categoryId;select.dispatchEvent(new Event("change", { bubbles: true }));}
    }
    trigger?.click();
    if (screen === "game" && options.mode) setTimeout(() => document.querySelector(`.mode-btn[data-mode="${options.mode}"]`)?.click(), 80);
  }
  if(options.history !== false) {
    ...
    historyPush({hw9:true,focus:screen,returnTab:app.returnTab},`#/focus/${screen}${query}`); app.lastRoute=location.hash;
  }
  setTimeout(syncLegacyVisibility, 0);
}
function goHomeFromLegacy({ history = true } = {}) {
  const bridge = legacyBridge();
  if (bridge?.navigate) bridge.navigate("home"); else document.querySelector('[data-nav="home"]')?.click();
  document.body.classList.remove("hw9-legacy-active");app.root.hidden = false;
...
function activeLegacyScreen() {
  for (const [name, id] of Object.entries(LEGACY_SCREENS)) if (document.getElementById(id)?.classList.contains("active")) return name;
  return "";
}
function syncLegacyVisibility() {
  const active = activeLegacyScreen(), season2Open = document.querySelector("#season2Overlay")?.hidden === false;
  if (active) {
    app.lastLegacyScreen = active; document.body.classList.add("hw9-legacy-active"); document.body.dataset.hw9LegacyScreen = active; app.root.hidden = true; enhanceLegacyChrome(active);
  } else {
    document.body.classList.remove("hw9-legacy-active"); delete document.body.dataset.hw9LegacyScreen; if (!season2Open) app.root.hidden = false;
  }
  document.body.classList.toggle("hw9-season2-open", season2Open);
  app.main.inert=season2Open;app.nav.inert=season2Open;app.root.querySelector('.hw9-header').inert=season2Open;
  const legacyShell=document.querySelector('body > .app-shell'); if(legacyShell) legacyShell.inert=season2Open;
  if(season2Open&&!app.season2WasOpen) app.focusBeforeSeason2=document.activeElement;
  if(!season2Open&&app.season2WasOpen) {app.focusBeforeSeason2?.focus?.({preventScroll:true});refreshSnapshot({force:true});}
...
function enhanceLegacyChrome(screen) {
  ...
  const heading = root.querySelector(".screen-head h2"); if (heading && titleMap[screen]) heading.textContent = titleMap[screen];
  const iconMap = { cardSpeakBtn: "volume", newQuestionBtn: "sync", refreshRankBtn: "sync", petShopBtn: "shop", syncBtn: "sync" };
  Object.entries(iconMap).forEach(([id, name]) => {const button = document.getElementById(id);if (button) button.innerHTML = icon(name);});
  if (screen === "game") {
    const hint = document.createElement("p");hint.id = "kidsPlayHint";hint.className = "kids-play-hint";
    root.querySelector("#gameBox").before(hint);refreshPlayHint();
    const observer = new MutationObserver(refreshPlayHint);
    root.querySelectorAll(".mode-btn").forEach(button => observer.observe(button, {attributes:true, attributeFilter:["class"]}));
  }
  if (screen === "manage") document.querySelector("#manageScreen .reward-admin-panel")?.classList.toggle("hw9-admin-allowed", app.claims.admin);
}
function watchScreens() {
  ...
  targets.forEach((target) => app.screenObserver.observe(target, { attributes: true, attributeFilter: ["class", "hidden"] }));syncLegacyVisibility();
}
function watchDialogs() {
  app.dialogObserver = new MutationObserver(() => {const hasOpen = Boolean(document.querySelector("dialog[open]"));document.body.classList.toggle("hw9-dialog-open", hasOpen);});
...
function speakWord(word) {
  ...
  speechSynthesis.cancel();const utterance = new SpeechSynthesisUtterance(text);utterance.lang = "en-US";utterance.rate = 0.82;speechSynthesis.speak(utterance);
}
function toggleSound() {document.querySelector("#soundToggle")?.click();setTimeout(() => refreshSnapshot({ force: true }), 80);}
...
function handleRootClick(event) {
  ...
  if (jump) {setTab(jump);return;}
  const legacy = event.target.closest("[data-hw9-legacy]")?.dataset.hw9Legacy;
  if (legacy) {openLegacy(legacy);return;}
  const game = event.target.closest("[data-hw9-game]")?.dataset.hw9Game;
  if (game) {
    const categoryId = app.main.querySelector("[data-hw9-game-category]")?.value || availableCategory(app.snapshot);
    openLegacy("game", { mode: game, categoryId });return;
  }
  const category = event.target.closest("[data-hw9-category]")?.dataset.hw9Category;
  if (category) {openLegacy("card", { categoryId: category });return;}
  const speak = event.target.closest("[data-hw9-speak]")?.dataset.hw9Speak;
  if (speak) {speakWord(speak);return;}
  ...
  const actions = {
    ...
    "clear-search": () => {
      ...
      const input=app.main.querySelector('[data-hw9-word-search]'),select=app.main.querySelector('[data-hw9-word-category]');
      if(input) {input.value='';input.focus();} if(select) select.value='all';refreshWordResults();
    },
    "claim-mission": claimMission, "close-parent": closeParentGate
  };
  actions[action]?.();
}
function handleRootInput(event) {
  if(event.target.matches('[data-hw9-word-search]')) {app.wordQuery=event.target.value;app.wordLimit=60;if(!event.isComposing) refreshWordResults();}
}
function handleRootChange(event) {
  if(event.target.matches('[data-hw9-word-category]')) {app.wordCategory=event.target.value;app.wordLimit=60;refreshWordResults();}
  if(event.target.matches('[data-hw9-game-category]')) legacyBridge()?.selectCategory?.(event.target.value);
}
function parentRecord() {return safeJson(localStorage.getItem(PARENT_GATE_KEY), null);}
function randomSalt() {const bytes = crypto.getRandomValues(new Uint8Array(16));return btoa(String.fromCharCode(...bytes));}
async function hashPin(pin, salt) {
  const data = new TextEncoder().encode(`${salt}:${pin}`);
  const digest = await crypto.subtle.digest("SHA-256", data);return btoa(String.fromCharCode(...new Uint8Array(digest)));
}
function openParentGate(onGranted) {
  ...
  dialog.querySelector("[data-hw9-parent-description]").textContent = setup ? "이 기기에서 보호자 도구를 열 때 사용할 숫자 PIN을 만들어 주세요." : "단어장 편집과 데이터 관리 화면을 열어요.";
  dialog.querySelector("[data-hw9-parent-confirm-wrap]").hidden = !setup;
  dialog.querySelector("[data-hw9-parent-pin]").value = "";dialog.querySelector("[data-hw9-parent-confirm]").value = "";dialog.querySelector("[data-hw9-parent-error]").textContent = "";
  dialog.showModal();setTimeout(() => dialog.querySelector("[data-hw9-parent-pin]").focus(), 60);
}
function closeParentGate() {app.parentAction=null;app.root.querySelector("#hw9ParentDialog")?.close();}
async function handleParentSubmit(event) {
  event.preventDefault();
  const dialog = app.root.querySelector("#hw9ParentDialog");
  const pin = dialog.querySelector("[data-hw9-parent-pin]").value.trim(), confirm = dialog.querySelector("[data-hw9-parent-confirm]").value.trim(), error = dialog.querySelector("[data-hw9-parent-error]");
  if (!/^\d{4,8}$/.test(pin)) {error.textContent = "숫자 4~8자리로 입력해 주세요.";return;}
  const submit=dialog.querySelector('[data-hw9-parent-submit]');
  if(submit.disabled) return;
  submit.disabled=true;
  try {
    if (dialog.dataset.mode === "setup") {
      if (pin !== confirm) {error.textContent = "두 PIN이 서로 달라요.";return;}
...
function grantParentAccess() {
  window.HEATHER_PARENT_GATE_GRANTED = true;legacyBridge()?.setManageGranted?.(true);
  const continuation=app.parentAction;app.parentAction=null;
...
async function detectClaims() {
  ...
  try {
    const [{ getApps }, { getAuth, getIdTokenResult }] = await Promise.all([import("https://www.gstatic.com/firebasejs/10.12.5/firebase-app.js"),import("https://www.gstatic.com/firebasejs/10.12.5/firebase-auth.js")]);
    const instance = getApps()[0];if (!instance) return;
    const auth = getAuth(instance);
    for (let attempt = 0; attempt < 40 && !auth.currentUser; attempt += 1) await new Promise((resolve) => setTimeout(resolve, 100));
    if (!auth.currentUser) return;
    const token = await getIdTokenResult(auth.currentUser, false);
    app.claims.admin = token.claims.admin === true;app.claims.guardian = token.claims.guardian === true || app.claims.admin;
    window.HEATHER_FIREBASE_ADMIN = app.claims.admin;window.HEATHER_FIREBASE_GUARDIAN = app.claims.guardian;
    document.body.classList.toggle("hw9-admin-claim", app.claims.admin);
  } catch (error) {console.info("Heather Word role check unavailable", error);}
}
function interceptLegacyManage(event) {
  ...
  event.preventDefault();event.stopImmediatePropagation();openParentGate();
}
function interceptLegacyBack(event) {
  ...
  event.preventDefault();event.stopImmediatePropagation();goHomeFromLegacy();
}
function enhanceSeason2Overlay() {
  const root = document.querySelector("#season2Overlay");
  if (!root || root.dataset.hw9Enhanced === "true") return;
  root.dataset.hw9Enhanced = "true";
  const close = root.querySelector('[data-s2-action="close"]');if (close) close.innerHTML = icon("back");
  const nav = { adventure: ["sparkles", "모험"], collection: ["collection", "도감"], weekly: ["report", "주간"], settings: ["settings", "설정"] };
  Object.entries(nav).forEach(([view, [iconName, label]]) => {const button = root.querySelector(`[data-s2-view="${view}"]`);if (button) button.innerHTML = `<span>${icon(iconName)}</span><b>${label}</b>`;});
}
...
function bindEvents() {
  app.root.addEventListener("click", handleRootClick);app.root.addEventListener("input", handleRootInput);app.root.addEventListener("compositionend", handleRootInput);
  app.root.querySelector("#hw9ParentDialog").addEventListener("cancel",()=>{app.parentAction=null;});
  document.addEventListener("keydown",trapSeasonFocus,true);app.root.addEventListener("change", handleRootChange);app.root.querySelector("[data-hw9-parent-form]").addEventListener("submit", handleParentSubmit);
  window.addEventListener("heather:state-change", scheduleRefresh);window.addEventListener("heather:legacy-render", scheduleRefresh);window.addEventListener("heather:legacy-ready",scheduleRefresh);
  window.addEventListener("storage", (event) => { if (event.key === LOCAL_KEY) refreshSnapshot({ force: true }); });
  window.addEventListener("online", () => refreshSnapshot({ force: true }));window.addEventListener("offline", () => refreshSnapshot({ force: true }));
  window.addEventListener('heather:season2-open', event => {
    if(app.handlingHistory) return;
    const view=event.detail?.view||'adventure',hash=`#/season2/${view}`;
  ...
  window.addEventListener('heather:season2-close', () => {
    ...
    history.replaceState({hw9:true,tab},'',`#/${tab}`);app.lastRoute=location.hash;setTab(tab,{history:false,focus:false});
  });
  window.addEventListener("popstate", handleHistory);window.addEventListener("hashchange", handleHistory);window.addEventListener("heather:parent-gate-request", openParentGate);
  document.addEventListener("click", interceptLegacyManage, true);document.addEventListener("click", interceptLegacyBack, true);
  document.addEventListener("click", () => setTimeout(enhanceSeason2Overlay, 0), true);
}
function installGlobalApi() {
  window.HeatherWordUI = Object.freeze({version: UI_V9_VERSION,setTab,refresh: () => refreshSnapshot({ force: true }),openLegacy,openAdventure,requestParentAccess: openParentGate,backToShell: () => goHomeFromLegacy(),getSnapshot: () => typeof structuredClone === "function" ? structuredClone(app.snapshot) : JSON.parse(JSON.stringify(app.snapshot))});
}
async function boot() {
  const ready = await waitForBaseApp();
  if (!ready) {console.error("Heather Word base app did not become ready");return;}
  installStorageSignal();app.root = mountShell();app.main = app.root.querySelector("#hw9Content");app.nav = app.root.querySelector(".hw9-tabbar");
  document.documentElement.classList.add("hw9-ready");document.body.classList.add("hw9-shell-ready");document.body.dataset.hw9Version = UI_V9_VERSION;
  bindEvents();watchScreens();watchDialogs();enhanceSeason2Overlay();refreshSnapshot({ force: true });installGlobalApi();detectClaims();
  ...
  document.querySelector('#hwBoot')?.remove();document.querySelector('#hwBootStyle')?.remove();handleHistory();
  if (!location.hash || !location.hash.startsWith("#/")) history.replaceState({ hw9: true, tab: app.activeTab }, "", `#/${app.activeTab}`);
  window.dispatchEvent(new CustomEvent("heather:ui-v9-ready", { detail: { version: UI_V9_VERSION } }));
}
boot().catch((error) => {console.error("Heather Word UI v9 failed", error);document.querySelector("#hwBoot")?.classList.add("has-error");});
