...
|F29|시즌2 데이터 호환|schema8 마이그레이션·레거시 경로·리비전 병합|season2-core.js, firebase-config.js|player.season2 / progress.__season2|기존 core/계약 테스트·데이터 보존|
...
## 변경 후 실행 기록
- [x] 기존 단위/계약 + 신규 회귀 테스트: 44개 통과
- [x] 기존 및 확장 브라우저 시나리오: 69개 확장 확인 + 27개 시각/포인터 확인
- [x] 360/390/430/768/1440 등 반응형 캡처와 시각 검토: 56개 PNG 산출
- [x] 점수·알고리즘·저장 키 보존 비교: 194개 기존 함수 및 5개 보호 파일 바이트 비교
- [x] 오류/경고 및 HTTP 실패 분리 기록: 실행 범위와 수집 제한은 QUALITY_REPORT.md 참조
- [x] Lighthouse 네 카테고리 실제 수치: 86 / 100 / 100 / 100
- [x] Pages 하위 경로·hash·메타·404 파일 검사; 실제 운영 배포 후 검사는 별도
- [ ] Firebase 권한·실기기 음성/가상키보드 수동 승인 (배포 전 필요)

## 최종 실행 범위 (2026-09-05)

검증한 앱 소스: `c4e39d23b9f665759eb8176cbe5af795580f4b9d`, Actions run `33938178871`. 이후 문서 정리 커밋은 앱 실행 파일을 변경하지 않는다.

|기능 ID|실제 자동 확인|남은 확인|
|---|---|---|
|F01–F05|LOCAL 첫 실행·기존 데이터, 카테고리, 카드/알아요 진입과 학습 데이터 유지|실제 클라우드·이름/발음·카드의 모든 분기|
|F06–F11|네 게임 10문제 완료, 쓰기 오답→피드백→다음, 결과 화면|모든 모드의 모든 오답/건너뛰기 조합|
|F12–F13|표시용 진행률 계약·기존 계산 함수 해시 보존|일자 경계와 미션 보상 전체 수동 시나리오|
|F14|151개 단어 전체 접근·검색 노드/포커스 유지·한글 조합|실제 모바일 키보드|
|F15–F18|단어 추가·일괄 추가·새로고침 후 유지; 기존 파서/관리 함수 해시 보존|삭제·복원·손상 백업·카테고리 삭제 전체 분기|
|F19–F26|LOCAL 랭킹·도감·펫·드레스룸·상점·관리 화면 진입; 로직 보존|구매/장착/먹이/이미지 저장 전체 흐름; 실제 상품권 처리는 미실행|
|F27–F28|PIN 설정·틀린 PIN·재잠금·리포트 잠금·규칙 파일 보존|실제 Firebase 권한·claim·다중 사용자|
|F29–F33|기존 core tests, 21문제 4단계 모험 완료, 진행 중 세션 재로드|전체 난이도/보스 힌트/날짜 변경 분기|
|F34–F37|기존 스타터·보상·진화 관련 core tests와 60개 SVG 검사|모든 필터·캐릭터·장착·부화 UI 조합|
|F38–F39|모션 줄이기 설정, PIN 리포트 접근, JSON/CSV 버튼 유지|실제 음성·내보내기 파일 내용·보조기기|
|F40|홈 7개 크기·5탭 전체 5개 크기, 하위 경로, 게임 모드 새로고침, dialog Tab 격리, 실제 포인터 히트 검사|물리 기기·Safari/Firefox·전체 history 조합·실제 Pages 배포 후 검사|

**한 행의 일부 자동 검증이 그 기능의 모든 분기 통과를 의미하지 않는다.** 구현 보존과 실행 확인을 구분해 출시 승인에 사용한다.
